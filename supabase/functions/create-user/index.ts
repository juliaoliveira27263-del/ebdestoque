...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { email, password, name, role, phone } = body as { email: string; password: string; name: string; role: string; phone?: string };
    ...
    if (newUser.user) {
      await supabase.from('profiles').upsert({ id: newUser.user.id, name, role, phone: phone || null, active: true });
    }
