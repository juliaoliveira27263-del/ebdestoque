...
export default function EsqueciSenha() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(false);
    if (error) { toast.error(error); } else { setSent(true); toast.success('Email de recuperação enviado!'); }
  };
    ...
    <div className={`min-h-screen flex items-center justify-center p-4 ${isDark ? 'bg-dark-950' : 'bg-gray-50'}`}>
      ...
      <div className={`w-full max-w-md p-8 rounded-2xl shadow-xl ${isDark ? 'bg-dark-900 border border-dark-800' : 'bg-white border border-gray-200'}`}>
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className={`text-sm text-center mt-2 ${isDark ? 'text-dark-400' : 'text-gray-500'}`}>{sent ? 'Verifique seu email para redefinir a senha.' : 'Digite seu email para receber o link de recuperação.'}</p>
        </div>
        {!sent ? (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={loading} className="w-full py-2.5 rounded-xl bg-primary text-white font-semibold hover:bg-primary-700 transition-colors disabled:opacity-50">{loading ? 'Enviando...' : 'Enviar link de recuperação'}</button>
          </form>
        ) : (
          <div className="text-center"><Link to="/login" className="inline-block py-2.5 px-6 rounded-xl bg-primary text-white font-semibold hover:bg-primary-700 transition-colors">Voltar ao login</Link></div>
        )}
      </div>
    </div>
  );
