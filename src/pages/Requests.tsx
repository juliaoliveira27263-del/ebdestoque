...
export default function Requests() {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase.from('products').select('*, industry:industries(*)').eq('active', true).order('name');
  ...
  const industryNames: Record<string, string> = useMemo(() => {
    ...
    products.forEach((p) => { names[p.industry_id ?? 'no-industry'] = p.industry?.name?.trim() ?? 'Sem Indústria'; });
    return names;
  }, [products]);
    ...
    <div className="p-4 lg:p-6 max-w-7xl mx-auto space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} className="input pl-10" placeholder="Buscar solicitações..." />
        </div>
                ...
                <button onClick={() => setExpandedRequest(isExpanded ? null : req.id)} className="w-full flex items-center gap-3 p-4 hover:bg-dark-700/50 transition-colors">
                  ...
                  <div className="flex-1 text-left min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-white font-medium text-sm">{req.profile?.name ?? 'Usuário'}</p>
                      <span className={statusConfig[req.status].badge}><StatusIcon className="w-3 h-3 mr-1" />{statusConfig[req.status].label}</span>
                    </div>
            ...
            {Object.entries(industryNames).map(([indId, indName]) => {
              ...
              return (
                <div key={indId} className="border border-dark-700 rounded-lg overflow-hidden">
                  <button type="button" onClick={() => setExpandedIndustry(isExp ? null : indId)} className="w-full flex items-center gap-2 p-3 hover:bg-dark-700/50 transition-colors">
                    {isExp ? <ChevronDown className="w-4 h-4 text-primary-500" /> : <ChevronRight className="w-4 h-4 text-dark-400" />}
                    ...
                    <div className="border-t border-dark-700 divide-y divide-dark-700/50">
                      {indProducts.map((product) => (
                        <div key={product.id} className="flex items-center gap-3 p-2.5">
                          {product.image_url ? <img src={product.image_url} alt={product.name} className="w-9 h-9 rounded-lg object-cover shrink-0" loading="lazy" /> : <div className="w-9 h-9 rounded-lg bg-dark-600 flex items-center justify-center shrink-0"><Package className="w-4 h-4 text-dark-400" /></div>}
                          <div className="flex-1 min-w-0"><p className="text-white text-sm truncate">{product.name}</p><p className="text-dark-400 text-xs">Estoque: {product.stock_quantity} {product.unit}</p></div>
                          <div className="flex items-center gap-1 shrink-0">
                            <button type="button" onClick={() => { const c = cart[product.id] ?? 0; if (c > 0) setCart((p) => ({ ...p, [product.id]: c - 1 })); }} className="w-7 h-7 rounded-md bg-dark-700 text-white hover:bg-dark-600 flex items-center justify-center">−</button>
                            <span className="w-8 text-center text-white text-sm font-medium">{cart[product.id] ?? 0}</span>
                            <button type="button" onClick={() => { const c = cart[product.id] ?? 0; setCart((p) => ({ ...p, [product.id]: c + 1 })); }} className="w-7 h-7 rounded-md bg-primary-600 text-white hover:bg-primary-700 flex items-center justify-center">+</button>
                          </div>
                        </div>
                      ))}
                    </div>
