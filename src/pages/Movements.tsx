...
export default function Movements() {
  ...
  const filteredMovements = useMemo(() => {
    ...
    return movements.filter((m) => m.product?.name?.toLowerCase().includes(q) || m.profile?.name?.toLowerCase().includes(q) || m.reason?.toLowerCase().includes(q));
  }, [movements, search]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setSubmitting(true); setError(null);
    const product = products.find((p) => p.id === form.product_id);
    if (!product) { setError('Selecione um produto'); setSubmitting(false); return; }
    const qty = parseInt(form.quantity);
    if (qty <= 0) { setError('Quantidade deve ser maior que zero'); setSubmitting(false); return; }
    try {
            ...
            return (
              <div key={mov.id} className="card p-3 flex items-center gap-3">
                ...
                <div className="text-right shrink-0">
                  <span className={`text-lg font-bold ${typeConfig[mov.type].color}`}>{mov.type === 'in' ? '+' : mov.type === 'out' ? '−' : '='}{mov.quantity}</span>
