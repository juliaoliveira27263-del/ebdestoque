...
import { ArrowLeft, Package, Check, AlertCircle, Minus, Plus } from 'lucide-react';
...
export default function IndustriaMateriais() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const request = requestData as { id: string };
    await supabase.from('request_items').insert({
      request_id: request.id, product_id: selectedProduct.id, quantity, industry_id: id,
    });
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
      ...
      {products.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          {products.map((product) => (
            <div key={product.id} className={`rounded-2xl border overflow-hidden transition-all duration-300 hover:shadow-lg ${
              isDark ? 'bg-dark-900 border-dark-800' : 'bg-white border-gray-200 shadow-sm'
            }`}>
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  {product.stock_quantity === 0 && (
                    <span className="text-xs text-error-500 font-medium">Esgotado</span>
                  )}
                </div>
                <button
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
          ...
          <div className={`relative w-full max-w-md rounded-2xl shadow-2xl border animate-scale-in max-h-[90vh] overflow-y-auto ${
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <input type="number" value={quantity} onChange={(e) => setQuantity(Math.max(1, Number(e.target.value)))} min={1} max={selectedProduct.stock_quantity}
