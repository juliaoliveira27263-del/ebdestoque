...
export default function Cadastro() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(false);
    if (error) { toast.error(error); } else { toast.success('Conta criada com sucesso! Faça login para continuar.'); navigate('/login'); }
  };
    ...
    <div className={`min-h-screen flex items-center justify-center p-4 ${isDark ? 'bg-dark-950' : 'bg-gray-50'}`}>
      ...
      <div className={`w-full max-w-md p-8 rounded-2xl shadow-xl ${isDark ? 'bg-dark-900 border border-dark-800' : 'bg-white border border-gray-200'}`}>
        <div className="flex flex-col items-center mb-8">
          ...
          <p className={`text-sm ${isDark ? 'text-dark-400' : 'text-gray-500'}`}>EBD Petrolina - Controle de Estoque</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className={`block text-sm font-medium mb-1.5 ${isDark ? 'text-dark-200' : 'text-gray-700'}`}>Nome</label>
            <div className="relative">
              <User className={`absolute left-3 top-1/2 -translate-y-1/2 ${isDark ? 'text-dark-400' : 'text-gray-400'}`} size={18} />
              <input type="text" value={name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setName(e.target.value)} required className={`w-full pl-10 pr-4 py-2.5 rounded-xl border outline-none transition-colors ${isDark ? 'bg-dark-800 border-dark-700 text-white focus:border-primary' : 'bg-gray-50 border-gray-200 text-gray-900 focus:border-primary'}`} placeholder="Seu nome completo" />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPassword(e.target.value)} required minLength={6} className={`w-full pl-10 pr-10 py-2.5 rounded-xl border outline-none transition-colors ${isDark ? 'bg-dark-800 border-dark-700 text-white focus:border-primary' : 'bg-gray-50 border-gray-200 text-gray-900 focus:border-primary'}`} placeholder="Mínimo 6 caracteres" />
          ...
          <button type="submit" disabled={loading} className="w-full py-2.5 rounded-xl bg-primary text-white font-semibold hover:bg-primary-700 transition-colors disabled:opacity-50">{loading ? 'Criando conta...' : 'Criar conta'}</button>
        </form>
        <div className="mt-6 text-center"><Link to="/login" className={`text-sm ${isDark ? 'text-dark-300 hover:text-white' : 'text-gray-600 hover:text-gray-900'}`}>Já tem conta? Faça login</Link></div>
      </div>
    </div>
  );
}
