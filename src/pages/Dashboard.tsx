...
export default function Dashboard() {
  ...
  const stockByIndustry = useMemo(() =>
    industries.map((ind) => ({
      name: ind.name.trim(),
      estoque: products.filter((p) => p.industry_id === ind.id).reduce((s, p) => s + p.stock_quantity, 0),
    })), [products, industries]);
  ...
  const requestsByPeriod = useMemo(() => {
    ...
    displayRequests.forEach((r) => {
      const key = new Date(r.created_at).toLocaleDateString('pt-BR', { weekday: 'short' });
  ...
  const topUsers = useMemo(() => {
    ...
    allRequests.forEach((r) => {
      ...
      if (!counts[r.user_id]) counts[r.user_id] = { name: uname, count: 0, items: 0 };
        ...
        <div className="card p-5">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={stockByIndustry}>
              ...
              <Bar dataKey="estoque" fill="#C00000" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="card p-5">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={requestsByPeriod}>
              ...
              <Line type="monotone" dataKey="solicitacoes" stroke="#C00000" strokeWidth={2} dot={{ fill: '#C00000', r: 4 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
