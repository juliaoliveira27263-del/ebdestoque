...
export default function RedefinirSenha() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    else { setSuccess(true); setSubmitting(false); setTimeout(() => navigate('/login'), 3000); }
  };
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type={showPassword ? 'text' : 'password'} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="input pl-10 pr-10" placeholder="Mínimo 8 caracteres" required minLength={8} autoComplete="new-password" autoFocus />
                ...
                <div>
                  ...
                  <div className="relative">
                    <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-dark-400" />
                    <input type={showConfirm ? 'text' : 'password'} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="input pl-10 pr-10" placeholder="Repita a nova senha" required minLength={8} autoComplete="new-password" />
                  ...
                  {confirmPassword && newPassword !== confirmPassword && <p className="text-error-500 text-xs mt-1">As senhas não coincidem.</p>}
                </div>
                {error && (
                ...
                <button type="submit" disabled={submitting} className="btn-primary w-full">
                  {submitting ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <>Atualizar senha <KeyRound className="w-4 h-4" /></>}
                </button>
              </form>
              <div className="mt-6 text-center">
                <Link to="/login" className="inline-flex items-center gap-1 text-sm text-dark-400 hover:text-white transition-colors">
