...
export default function Industries() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = { name: form.name, cnpj: form.cnpj || null, contact_name: form.contact_name || null, contact_email: form.contact_email || null, contact_phone: form.contact_phone || null, address: form.address || null, active: form.active };
    try {
      if (editing) { const { error } = await supabase.from('industries').update(payload).eq('id', editing.id); if (error) throw error; }
      else { const { error } = await supabase.from('industries').insert(payload); if (error) throw error; }
      setModalOpen(false); await fetchIndustries();
    } catch (err: any) { setError(err.message); } finally { setSubmitting(false); }
  };

  const handleDelete = async () => {
    if (!deleteId) return;
    const { error } = await supabase.from('industries').delete().eq('id', deleteId);
          ...
          <div key={ind.id} className="card p-4">
            <div className="flex items-start justify-between gap-2 mb-3">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0"><p className="text-white font-semibold truncate">{ind.name.trim()}</p>{ind.cnpj && <p className="text-dark-400 text-xs">CNPJ: {ind.cnpj}</p>}</div>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => openEdit(ind)} className="p-1.5 rounded-lg text-dark-400 hover:text-primary-500 hover:bg-dark-700 transition-colors"><Edit2 className="w-4 h-4" /></button>
                <button onClick={() => setDeleteId(ind.id)} className="p-1.5 rounded-lg text-dark-400 hover:text-error-500 hover:bg-dark-700 transition-colors"><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
            <div className="space-y-1 text-sm text-dark-300">
              {ind.contact_name && <p className="truncate">{ind.contact_name}</p>}
              {ind.contact_phone && <p className="flex items-center gap-2 truncate"><Phone className="w-3.5 h-3.5 text-dark-400 shrink-0" />{ind.contact_phone}</p>}
              {ind.contact_email && <p className="flex items-center gap-2 truncate"><Mail className="w-3.5 h-3.5 text-dark-400 shrink-0" />{ind.contact_email}</p>}
              {ind.address && <p className="flex items-center gap-2 truncate"><MapPin className="w-3.5 h-3.5 text-dark-400 shrink-0" />{ind.address}</p>}
            </div>
            <div className="mt-3">{ind.active ? <span className="badge-success">Ativa</span> : <span className="badge-neutral">Inativa</span>}</div>
          </div>
        ))}
      </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar Indústria' : 'Nova Indústria'} size="md">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div><label className="label">Telefone</label><input type="text" value={form.contact_phone} onChange={(e) => setForm((p) => ({ ...p, contact_phone: e.target.value }))} className="input" /></div>
            <div><label className="label">E-mail</label><input type="email" value={form.contact_email} onChange={(e) => setForm((p) => ({ ...p, contact_email: e.target.value }))} className="input" /></div>
          </div>
