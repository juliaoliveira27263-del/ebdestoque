...
import { Users as UsersIcon, Plus, Edit2, Trash2, AlertCircle, Shield, UserCheck, UserX, Mail } from 'lucide-react';
...
export default function Users() {
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ name: '', email: '', role: 'vendedor', phone: '', active: true });
    setError(null);
    setModalOpen(true);
  };
  ...
  const openEdit = (p: Profile) => {
    setEditing(p);
    setForm({ name: p.name, email: '', role: p.role, phone: p.phone ?? '', active: p.active });
    setError(null);
    setModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        ...
        if (!form.email) {
          setError('E-mail é obrigatório para novos usuários');
          setSubmitting(false);
          return;
        }
    ...
    <div className="p-4 lg:p-6 max-w-7xl mx-auto space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-dark-400 text-sm mt-1">{profiles.length} usuário(s) cadastrado(s)</p>
        </div>
        ...
        {profiles.map((p) => (
          <div key={p.id} className="card p-4">
            <div className="flex items-start justify-between gap-2 mb-3">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  {p.phone && <p className="text-dark-400 text-xs">{p.phone}</p>}
                </div>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => openEdit(p)} className="p-1.5 rounded-lg text-dark-400 hover:text-primary-400 hover:bg-dark-700 transition-colors">
                  <Edit2 className="w-4 h-4" />
                </button>
                {p.id !== currentUser?.id && (
                  <button onClick={() => setDeleteId(p.id)} className="p-1.5 rounded-lg text-dark-400 hover:text-error-500 hover:bg-dark-700 transition-colors">
            ...
            <div className="flex items-center gap-2">
              ...
              {p.active ? (
                <span className="badge-success"><UserCheck className="w-3 h-3 mr-1" />Ativo</span>
              ) : (
                <span className="badge-neutral"><UserX className="w-3 h-3 mr-1" />Inativo</span>
              )}
            </div>
          </div>
        ))}
          ...
          {!editing && (
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={form.email} onChange={(e) => setForm((p) => ({ ...p, email: e.target.value }))} className="input pl-10" required />
              </div>
            </div>
          )}
          <div>
            <label className="label">Perfil *</label>
            <select value={form.role} onChange={(e) => setForm((p) => ({ ...p, role: e.target.value as UserRole }))} className="input" required>
      ...
      <Modal open={!!deleteId} onClose={() => setDeleteId(null)} title="Excluir Usuário" size="sm">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => { setDeleteId(null); setError(null); }} className="btn-secondary">Cancelar</button>
        </div>
      </Modal>
    </div>
  );
