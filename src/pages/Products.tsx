...
const PLACEHOLDER_IMAGE = 'data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCA0OCA0OCI+PHJlY3Qgd2lkdGg9IjQ4IiBoZWlnaHQ9IjQ4IiBmaWxsPSIjMmQyZTM0Ii8+PHBhdGggZD0iTTI0IDE0TDI0IDM0TTE0IDI0TDM0IDI0IiBzdHJva2U9IiM2ZDZlNzgiIHN0cm9rZS13aWR0aD0iMiIvPjwvc3ZnPg==';
...
export default function Products() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((p) =>
      ...
      p.industry?.name.toLowerCase().includes(q)
    );
  }, [products, search]);
  ...
  const productsByIndustry = useMemo(() => {
    ...
    filteredProducts.forEach((p) => {
      ...
      const indName = p.industry?.name?.trim() ?? 'Sem Indústria';
      if (!map[indId]) map[indId] = [];
      map[indId].push(p);
      // Store the industry name in a parallel map
      (map as any)[`name_${indId}`] = indName;
    });
    return map;
  }, [filteredProducts]);
          ...
          Object.entries(industryNames).map(([indId, indName]) => {
            ...
            return (
              <div key={indId} className="card overflow-hidden">
                <button
                  onClick={() => setExpandedIndustry(isExpanded ? null : indId)}
                ...
                >
                  ...
                  <div className="text-right">
                    <span className="text-dark-300 text-sm font-medium">
                      {indProducts.reduce((s, p) => s + p.stock_quantity, 0)} em estoque
                    </span>
                  </div>
                </button>
                      ...
                      <div key={product.id} className="flex items-center gap-3 p-3 hover:bg-dark-700/30 transition-colors">
                        {product.image_url ? (
                          <img
                            ...
                            alt={product.name}
                            className="w-12 h-12 rounded-lg object-cover shrink-0 bg-dark-600"
                            loading="lazy"
                          />
                        ) : (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                ...
                {industries.map((ind) => (
                  <option key={ind.id} value={ind.id}>{ind.name.trim()}</option>
                ))}
              </select>
            </div>
